...
export class SendCardToTeamMemberRule extends SimultaneousRule {
    getActivePlayerLegalMoves(player: number): MaterialMove<number, number, number>[] {
        return this.material(MaterialType.KitsuCard).location(LocationType.PlayerHand).player(player).moveItems({type: LocationType.PlayedKitsuCardSpot, player: player, rotation: KitsuCardRotation.FaceDown});
    }
    ...
    getMovesAfterPlayersDone(): MaterialMove<number, number, number>[] {
        ...
        if (leaderToken === undefined || leaderToken.location.player === undefined) {
            throw new Error("Missing leader token");
        }
        return [
            ...this.material(MaterialType.KitsuCard).location(LocationType.PlayedKitsuCardSpot).moveItems(item => ({type: LocationType.PlayerHand, player: this.getDestinationPlayer(item.location.player!), rotation: undefined})),

import { CustomMove, isCustomMoveType, MaterialMove, PlayerTurnRule, PlayMoveContext, RuleMove, RuleStep } from '@gamepark/rules-api'
...
export class RoundEndRule extends PlayerTurnRule<number, MaterialType, LocationType> {
  public onRuleStart(
    ...
    const victoriousPawn = this.material(MaterialType.KitsunePawn).location(LocationType.KitsunePawnSpotOnWisdomBoard).locationId(13).getItem<KitsunePawn>()

    if (victoriousPawn !== undefined) {
      const victoryCard = this.material(MaterialType.VictoryCard)
  ...
  public onCustomMove(move: CustomMove, _context?: PlayMoveContext): MaterialMove<number, MaterialType, LocationType>[] {
    if (isCustomMoveType<CustomMoveType>(CustomMoveType.PickRandomPlayer)(move)) {
      ...
      const nextLeader = victoriousPlayers[move.data as number]
      return [

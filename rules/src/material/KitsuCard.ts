...
export const canBePlayedWithProtectionToken = (cardId: KitsuCard): boolean =>
  !isSpecialCard(cardId) || (isSpecialCard(cardId) && getSpecialCardType(cardId) === KitsuCardSpecialType.WhiteKitsune)
...
export const isKitsuCardMaterial = (item: Partial<MaterialItem<number, LocationType>>): item is MaterialItem<number, LocationType, KitsuCard> =>
  kitsuCardIds.includes(item.id)

...
describe('RoundEnd rule tests', () => {
  ...
  ])('Given a team with a victory card, onRuleStart() should return an array containing only a EndGame rule move', ({ victoryCardTeam, kitsunePawnTeam }) => {
    //Given
  ...
  describe('2 players tests', () => {
    test.each([
      ...
      ({ winningTeam, expectedVictoryCardId }) => {
        // Given
        const gameBuilder = create2PlayersGameBuilder()
    ...
    ])('onCustomMove() should return a leadertoken MoveItem and a StartPlayerTurn rule move', ({ winningTeam, expectedPlayer }) => {
      // Given
      const gameBuilder = create2PlayersGameBuilder()
      gameBuilder.material(MaterialType.KitsunePawn).id(winningTeam).moveItem({
        type: LocationType.KitsunePawnSpotOnWisdomBoard,

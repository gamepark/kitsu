import { isMoveItemType, isStartPlayerTurn, ItemMoveType, MoveItem, MoveKind, RuleMoveType, StartPlayerTurn } from '@gamepark/rules-api'
...
describe('End fo trick - pick available power token tests', () => {
  ...
  test.each([
    ...
    'Given team already has a power token, onRuleStart() should return an array containing only a rule move to ' + 'the EndOfTrickDiscardCards rule',
    ({ givenPlayerWithPowerToken, givenPlayerToken }) => {
      // Given
      const gameBuilder = create2PlayersGameBuilder()
  ...
  test.each([
    ...
    'Given the available power tokens, getPlayerMoves() should return an array of move of each player ' + "token to each team's member clan card",
    ({ givenAvailablePowerTokens }) => {
      // Given
      const gameBuilder = create2PlayersGameBuilder()
      gameBuilder.setRule(RuleId.EndOfTrickPickAvailablePowerToken, 1)
      gameBuilder
  ...
  test(
    "Given a power token move to a team member's clan card, afterItemMove() should return an array containing " +
    ...
    () => {
      ...
      const ruleMoves = consequences.filter(isStartPlayerTurn<number, MaterialType, LocationType>).map((move) => move as StartPlayerTurn<number, RuleId>)
  ...
  test('Given an invalid move, afterItemMove() should return an array containing ' + 'only a rule move to the EndOfTrickDiscardCards', () => {
    // Given
    const gameBuilder = create2PlayersGameBuilder()
    gameBuilder.setRule(RuleId.EndOfTrickPickAvailablePowerToken, 1)
    const game = gameBuilder.build()

...
describe('End of trick - Move leader token rule tests', () => {
  describe('2-players tests', () => {
    test.each([
      ...
      {
        playerHands: [
          { player: 1 as 1 | 2, cardIds: [KitsuCard.Zenko4] },
          {
            player: 2 as 1 | 2,
      ...
      'Given non-empty player hands, onRuleStart() should return an array containing one item move for the ' +
        'leader token and one rule move to the EndOfTrickPickCardsRule',
      ({ playerHands }) => {
        // Given
    ...
    test(
      'Given empty player hands, onRuleStart() should return an array containing one item move for the ' +

  ...
  describe('Given a game with 2 players', () => {
    ...
    test.each([
      ...
      'Given discarded power tokens, onRuleStart() should return a consequences array with 3 moves, one for the' +
        ' kitsune pawns, one for the power tokens and a rule move to the next rule',
      ({ givenDiscardedPowerTokens, expectedTokenIndexes }) => {
        // Given
        ...
        gameBuilder
          ...
          .id<PowerToken>((id) => givenDiscardedPowerTokens.includes(id))
          .moveItems({

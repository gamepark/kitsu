import { isMoveItemType, isStartPlayerTurn, ItemMoveType, MoveItem, MoveKind, RuleMoveType, StartRule } from '@gamepark/rules-api'
...
describe('SelectKatanaTarget rule tests', () => {
  test.each([
    ...
    ({ givenPlayedCards, givenActivePlayer }) => {
      // Given
  ...
  ])('Given numeral cards already played, onRuleStart() should return an empty array ', ({ givenPlayedCards, givenActivePlayer }) => {
    // Given
    const gameBuilder = create2PlayersGameBuilderWithPlayedCards(givenPlayedCards)
    gameBuilder.setRule(RuleId.SelectKatanaTarget, givenActivePlayer)
    const game = gameBuilder.build()
  ...
  test.each([
    ...
    ({ givenPlayedCards, givenActivePlayer, expectedCardIds }) => {
      // Given
      const gameBuilder = create2PlayersGameBuilderWithPlayedCards(givenPlayedCards)
      gameBuilder.setRule(RuleId.SelectKatanaTarget, givenActivePlayer)
      const game = gameBuilder.build()
  ...
  test('Given face down cards, getPlayerMoves() should not return moves for face down cards', () => {
    ...
    const gameBuilder = create2PlayersGameBuilderWithPlayedCards([
      {
        player: 1,
        playedCardIds: [KitsuCard.Yako1_3, KitsuCard.Katana_1],
      },
      {
        player: 2,
        playedCardIds: [KitsuCard.Yako2_2, KitsuCard.Zenko6],
      },
    ])
  ...
  test.each([
    ...
    ({ givenPlayedCards, givenActivePlayer, givenRotatedCardId }) => {
      // Given
      const gameBuilder = create2PlayersGameBuilderWithPlayedCards(givenPlayedCards)
      gameBuilder.setRule(RuleId.SelectKatanaTarget, givenActivePlayer)
      const game = gameBuilder.build()

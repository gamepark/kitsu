...
export const EndOfTrickNoPickToken: FC<MoveComponentProps<MaterialMove<number, MaterialType, LocationType>>> = ({ context }) => {
  ...
  if (availableTokenNumber === 0) {
    if (me === actingPlayer) {
      return <Trans i18nKey="log.endOfTrick.pickPowerToken.noneAvailable.self" />
    }
    return <Trans i18nKey="log.endOfTrick.pickPowerToken.noneAvailable.other" values={{ player: actingPlayerName }} />
  }
  if (me === actingPlayer) {
    return <Trans i18nKey="log.endOfTrick.pickPowerToken.teamAlreadyHasToken.self" />
  }
  return (
    <Trans
      i18nKey="log.endOfTrick.pickPowerToken.teamAlreadyHasToken.other"

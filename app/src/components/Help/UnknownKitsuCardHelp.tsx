...
interface UnknownKitsuCardHelpProps {
  item: Omit<MaterialItem<number, LocationType, KitsuCard>, 'id'>
}
    ...
    case LocationType.KitsuCardDiscardSpotOnWisdomBoard:
      return (
        <>
          ...
          <h3>{t('help.kitsuCard.unknown.title')}</h3>
          <p>
            {t('help.kitsuCard.unknown.location.discard', {
              numberOfCards: rules?.material(MaterialType.KitsuCard).location(LocationType.KitsuCardDiscardSpotOnWisdomBoard).length,
            })}
          </p>
        </>
      )
    case LocationType.KitsuCardDeckSpotOnWisdomBoard:
      return (
        <>
          ...
          <h3>{t('help.kitsuCard.unknown.title')}</h3>
          <p>
            {t('help.kitsuCard.unknown.location.deck', {
              numberOfCards: rules?.material(MaterialType.KitsuCard).location(LocationType.KitsuCardDeckSpotOnWisdomBoard).length,
            })}
          </p>
        </>
      )

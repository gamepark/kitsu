...
export class KitsuTutorialSetup extends KitsuSetup {
  public setupMaterial(options: KitsuOptions): void {
    ...
    deck.shuffle()
    this.material(MaterialType.KitsuCard).id<KitsuCard>(KitsuCard.WhiteKitsune_1).moveItem({
      type: LocationType.KitsuCardDeckSpotOnWisdomBoard,
      x: 2,
    })
    this.material(MaterialType.KitsuCard).id<KitsuCard>(KitsuCard.WhiteKitsune_2).moveItem({
      type: LocationType.KitsuCardDeckSpotOnWisdomBoard,
      x: 2,
    })
  }
